...
@app.get('/api/databits/bits/list', name='get_api_databits_bits_list', \
        report_prompts=get_api_databits_bits_list_prompts,
        report_title='Data Bit List')
def get_api_databits_bits_list():
...
"""
    ...
    if tag != None:
        params['tag'] = tag
        wheres.append("(select count(*) from databits.tagbits where tag_id=%(tag)s and bit_id=bits.id)>0")
...
def put_api_databits_bit(bit_id):
    acc = api.table_from_tab2('bit', amendments=['id'], options=['caption', 'data', 'website', 'uname', 'pword'])
